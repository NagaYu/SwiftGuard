...
1. [Releases](https://github.com/your-org/SwiftGuard/releases) から `SwiftGuard-x.y.z.dmg` をダウンロード
2. DMG を開き、`SwiftGuard.app` を `Applications` へドラッグ
3. 起動 → フォルダ／ファイルをドラッグ＆ドロップ
...
```bash
git clone https://github.com/your-org/SwiftGuard.git
cd SwiftGuard
...
| `--no-color` | カラー出力を無効化 |
| `--max-chars <n>` | 1 ファイルあたりの最大監査文字数（既定 12000） |

**終了コード:** `0` = 問題なし / `1` = 重大な問題あり (`--strict`) または対象なし / `2` = モデル利用不可・パスエラー
